...
| Scraper scripts | Collect rates from interest.co.nz aggregator pages |
...
| Scraping     | Cheerio                                                   |
| Code quality | Oxlint, Oxfmt, TypeScript (`tsgo`), Fallow, and Bun tests |
...
| `apps/api/test/` | API, scraper, and data loader tests |
| `apps/docs/app/` | Next.js App Router routes for the docs site |
| `apps/docs/content/docs/` | Filesystem MDX documentation |
| `apps/docs/test/` | Docs content tests |
| `apps/web/` | Landing page built with TanStack Start |
| `.github/workflows/` | CI, deploy, scraping, monitoring, and Fallow workflows |

## Contribution Paths

- API changes: update route handlers, schemas, OpenAPI descriptions, and contract tests together. Write OpenAPI descriptions in ASD-STE100 Simplified Technical English.
- Scraper changes: update scraper normalization, validation, and D1 write behavior together.
- Docs changes: edit MDX files under `apps/docs/content/docs` and run `bun run check`.
