...
### Fetch Requests to the Same Zone

The landing page gets rates from the API during server rendering. The landing page Worker has the `global_fetch_strictly_public` compatibility flag. Without this flag, a `fetch()` to `www.ratesapi.nz` goes directly to the origin and does not go through the API Worker routes.
