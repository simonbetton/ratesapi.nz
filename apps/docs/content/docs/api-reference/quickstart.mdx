...
Use [`/llms.txt`](/llms.txt) to give an assistant a plain-text index of the documentation pages, and use [`/openapi/json`](/openapi/json) for request/response schemas or when your framework can generate tools from an OpenAPI contract.
...
- Use the [OpenAPI reference](/openapi) to find all parameters and response fields for each endpoint.
