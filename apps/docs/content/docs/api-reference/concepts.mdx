...
Historical snapshots are written only when scraped data changes: the scraper runs hourly, but history keeps at most one snapshot per dataset per UTC date; a later change on the same date replaces the earlier snapshot. Dates without a change have no historical row. Use each time-series response's `availableDates` array to see which dates actually have data instead of assuming a complete daily series.
