...
// Worker claims /api/v1/* and /openapi*, and the docs serve everything else.
// Locally each app runs on its own dev server (see the docs' local development
// page), so links point at those instead.
const productionOrigin = "https://ratesapi.nz";
...
export function siteOrigins(isDevelopment: boolean) {
  return {
    ...
    docs: isDevelopment ? localDocsOrigin : productionOrigin,
  };
}
